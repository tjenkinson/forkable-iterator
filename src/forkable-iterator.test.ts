...
describe('ForkableIterator', () => {
  describe('with a standard `Iterator`', () => {
    ...
    it('yields the correct items', () => {
      const child1: ForkableIterator<number, string> = fork(iterator);
  ...
  it('supports an AsyncIterator', async () => {
    const iterator: ForkableAsyncIterator<number, string> =
      buildForkableIterator(
        (async function* () {
          yield await 1;
          yield await 2;
    ...
    const child1: ForkableAsyncIterator<number, string> = fork(iterator);
    expect(await iterator.next()).toStrictEqual({ value: 1, done: false });
    expect(await child1.next()).toStrictEqual({ value: 1, done: false });
  });
});
